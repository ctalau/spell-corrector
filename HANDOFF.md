...
## 5. Status at handoff: NO TRAINED MODEL YET

The last run (pod `3gxtgufkzcvl40`, commit `09b7978`) got **all the way into the
full train and died at step 18 of 8,130 with a CUDA OOM** in backward. The pod is
terminated. There is no trained model and no BEA number yet.

**The OOM is fixed in the commit that adds this section — but the fix is
unverified on GPU. Your first run will be the test of it.**

What the run did establish, all reproducible:
...
Dataset statistics (`/tmp/run3/data_stats.json` from the fetched artifacts, and
regenerated by any build):
...
- it uses the **configured microbatch at maximum sequence length**, so peak
  ...
  of the card. Verified to reproduce #8; #9's guard is unverified on GPU.
...
- **`gold0_fraction` budgets against the target, not the produced count** (see
  §5), leaving easy examples over-represented at 76.6% vs BEA's 66.8%.
...
1. **Relaunch** (§4). Preflight will confirm the OOM fix in ~60 s. If it still
   peaks near the card, drop microbatch to 64 and raise accum to 8.
2. Let it run (~1h40m train + ~15 min benchmark, ~$2).
3. `python scripts/summarize_experiment.py`, then write `reports/EXPERIMENT.md`.
4. Only then consider the open items in §7 — none of them block a result.

If the result lands short of 75%, the decomposition in §8 tells you which lever
matters. Do not quietly widen the candidate generator to hit the number: that
was ruled out by the user, and the honest 72-75% result with the ceiling
explained is more useful than a number obtained by changing the task.
