...
GPU pods use `runpod/pytorch:2.4.0-py3.11-cuda12.4.1-devel-ubuntu22.04` (Python 3.11 so `hunspell==0.5.5` builds; matches Community hosts whose driver is CUDA 12.4). `scripts/runpod/setup_llm_judge.sh` then pip-installs **`torch==2.5.1+cu124`** (not unbounded `>=2.5.1`) into the venv, force-installs **`nvidia-cudnn-cu12`** (and other `nvidia-*` wheels) into the venv, puts those `nvidia/*/lib` dirs on **`LD_LIBRARY_PATH`** via `/workspace/llm-judge-env.sh`, and installs **`transformers>=5.5`** (no 5.15 cap).

Gemma-4 (`google/gemma-4-E2B-it`, `model_type` `gemma4`, landed in transformers 5.5) loads through `core_model_loading` → `distributed.sharding_utils` → `from torch.distributed.tensor import DTensor`. That export exists on PyTorch ≥2.5; torch 2.4 only has `torch.distributed._tensor`. Pinning `transformers>=5.5,<5.15` on image torch 2.4.1 (pod `rwfef0kegboxvk`, transformers 5.14.1) still hard-imported DTensor and died at model load after BEA prep (`bea_n_eligible=67844`). The follow-up (#15) installed `torch-2.6.0+cu124` on pod `9cfvu6gcwn11ks`; pip treated image `nvidia-cudnn-cu12==9.1.0.70` as already satisfied from system site-packages, then `import torch` failed with `libcudnn.so.9: cannot open shared object file`. Pin 2.5.1 (declares nvidia deps; same cudnn 9.1 as this image), reinstall those wheels into the venv, and source `LD_LIBRARY_PATH` in bootstrap — setup is a subprocess, so an export inside it does not survive. Do **not** use cu128 wheels or `runpod/pytorch:1.0.2-cu1281-torch280-ubuntu2404`: on these CUDA 12.4 hosts they silently fall back to CPU. Setup aborts if CUDA is unavailable after the torch upgrade, and asserts the DTensor + `transformers.models.gemma4` imports succeed before the run starts.
