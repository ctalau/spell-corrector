...
# ---- regexes -------------------------------------------------------------------------------------------------------
CAP = r"[A-Z0-9][\w.+#/&'-]*"                      # a capitalized label word ("Save", "XML", "Find/Replace")
...
FILE_PATTERNS = [
    ...
    ("filename", re.compile(rf"(?<![\w@/.-])(?P<m>[\w$~{{}}.-]*[\w}}]\.(?:{EXT}))(?![\w/-])", re.I)),
...
UI_STOP = {"I", "A", "An", "The", "This", "That", "These", "Those", "It", "If", "When", "For", "To", "In", "On", "You",
           "Oxygen", "Note", "Tip", "Important", "Attention", "Remember", "Result", "Step", "Figure", "Table", "See",
...
def linearize(block):
    ...
    def walk(el, free, own):
        ...
        if free and in_own and name in PATTERNS and el is not block:
        ...
        for a, b in spans:
            ...
            for kind, rx in pats:
                for m in rx.finditer(chunk):
                    ...
                    e = s + len(val)
                    if (s, e) in seen or not val.strip(): continue
                    if elem == "uicontrol":
                        words = val.split()
        ...
        if gold:
            ...
            for name, (s, e) in zip(names, mspans):
                ...
                yield {"file": str(path.relative_to(root)), "line": block.sourceline, "block": local(block.tag),
                       "element": name, "rule": "gold", "match": val, "context": text[:s] + "[[" + val + "]]" + text[e:]}
            continue
        text, spans = norm(raw, spans)
        if not spans: continue
        for elem, kind, val, s, e in candidates_in(text, spans):
            yield {
                "file": str(path.relative_to(root)), "line": block.sourceline, "block": local(block.tag),
                "element": elem, "rule": kind, "match": val,
                "context": text[:s] + "[[" + val + "]]" + text[e:],
            }
